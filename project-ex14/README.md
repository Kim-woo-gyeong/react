...
<pre>
1. this.state를 직접적으로 다루지 말아라.
2. this.state를 불변 객체로 취급해라
3. 컴포넌트 UI의 상태변경이 필요하면 항상 setState를 사용해서 변경해라

이유 설명해 줄 것 :
1. this.state를 직접 조작하는 것은 React의 상태 관리를 우회하는 것 -> 리액트 프로그래밍 모델에 반하는 일 -> 잠재적인 위험.
